...
namespace al
{
	...
	public:
		...
		SoloudSoundChannel(ISoundSystem &system,ISoundBuffer &buffer,SoLoud::WavInstance &&instance)
			: ISoundChannel{system,buffer},m_instance{std::move(instance)}
		{}
		SoloudSoundChannel(ISoundSystem &system,Decoder &decoder,SoLoud::WavInstance &&instance)
			: ISoundChannel{system,decoder},m_instance{std::move(instance)}
		{}
	...
	class SoloudListener
		: public IListener
	{
	public:
		SoloudListener(al::ISoundSystem &system)
			: IListener{system}
		{}
...
al::SoloudSoundSystem::SoloudSoundSystem(float metersPerUnit)
	: ISoundSystem{metersPerUnit}
{}

al::SoloudSoundSystem::~SoloudSoundSystem()
{
	m_soloud.deinit();
}
...
{
	return static_cast<SoloudError>(m_soloud.init());
}
...
void al::SoloudSoundChannel::SetGainRange(float minGain,float maxGain) {m_minGain = minGain; m_maxGain = maxGain;}
std::pair<float,float> al::SoloudSoundChannel::GetGainRange() const {return {m_minGain,m_maxGain};}
float al::SoloudSoundChannel::GetMinGain() const {return m_minGain;}float al::SoloudSoundChannel::GetMaxGain() const {return m_maxGain;}
void al::SoloudSoundChannel::SetDistanceRange(float refDist,float maxDist) {m_refDist = refDist; m_maxDist = maxDist;}
...
void al::SoloudSoundChannel::SetDirection(const Vector3 &dir)
{
	m_direction = dir;
}
Vector3 al::SoloudSoundChannel::GetDirection() const {return m_direction;}

void al::SoloudSoundChannel::SetOrientation(const Vector3 &at,const Vector3 &up) {m_at = at; m_up = up;}
std::pair<Vector3,Vector3> al::SoloudSoundChannel::GetOrientation() const {return {m_at,m_up};}

void al::SoloudSoundChannel::SetConeAngles(float inner,float outer) {m_innerConeAngles = inner; m_outerConeAngles = outer;}
std::pair<float,float> al::SoloudSoundChannel::GetConeAngles() const {return {m_innerConeAngles,m_outerConeAngles};}
void al::SoloudSoundChannel::SetOuterConeGains(float gain,float gainHF) {m_outerConeGain = gain; m_outerConeGainHf = gainHF;}
...
void al::SoloudSoundChannel::SetRolloffFactors(float factor,float roomFactor) {m_rolloffFactor = factor; m_roomRolloffFactor = roomFactor;}
...
void al::SoloudSoundChannel::SetRelative(bool bRelative)
{
	m_relative = bRelative;
}
...
void al::SoloudSoundChannel::SetStereoAngles(float leftAngle,float rightAngle) {m_stereoLeftAngle = leftAngle; m_stereoRightAngle = rightAngle;}
...
void al::SoloudSoundChannel::SetGainAuto(bool directHF,bool send,bool sendHF) {m_directGainHfAuto = directHF; m_sendGainAuto = send; m_sendGainHfAuto = sendHF;}
...
std::unique_ptr<al::IListener> al::SoloudSoundSystem::CreateListener()
{
	return std::make_unique<SoloudListener>(*this);
}
...
void al::SoloudListener::SetPosition(const Vector3 &pos)
{
	static_cast<SoloudSoundSystem&>(m_soundSystem).GetSoloudEngine().set3dListenerPosition(pos.x,pos.y,pos.z);
}
void al::SoloudListener::SetVelocity(const Vector3 &vel)
{
	static_cast<SoloudSoundSystem&>(m_soundSystem).GetSoloudEngine().set3dListenerVelocity(vel.x,vel.y,vel.z);
}
